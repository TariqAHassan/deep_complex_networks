...
from   keras.layers import Layer, AveragePooling2D, AveragePooling3D, add, Add, concatenate, Concatenate, Input, \
    Flatten, Dense, Convolution2D, BatchNormalization, Activation, Reshape, ConvLSTM2D, Conv2D
...
class SaveBestModel(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if val_acc > self.best_acc:
            ...
            # Filenames
            hdf5Filename = os.path.join(self.bestdir,
                                        "Bestmodel_{:06d}_{:.4f}_{:.4f}.hdf5".format(epoch + 1, val_acc, val_loss))
            yamlFilename = os.path.join(self.bestdir,
                                        "Bestmodel_{:06d}_{:.4f}_{:.4f}.yaml".format(epoch + 1, val_acc, val_loss))
...
def train(d):
    ...
    if isResuming:
        L.getLogger("entry").info("*** Reentering Training Loop @ Epoch {:5d} ***".format(initialEpoch + 1))
    else:
        L.getLogger("entry").info("***  Entering Training Loop  @ First Epoch ***")
    L.getLogger("entry").info("**********************************************")
